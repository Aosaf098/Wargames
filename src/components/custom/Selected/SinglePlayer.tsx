...
const SinglePlayer = ({ singlePlayer }: SuperstarProps) => {
  ...
  return (
    <>
      <div className="h-[200px] flex items-center justify-between gap-4 lg:px-10 px-4 bg-accent rounded-xl">
        <div className="flex items-center gap-4">
          <img className="lg:w-32 w-28 rounded-lg" src={image} alt="" />
          <div className="space-y-4">
            <h1 className="md:text-lg text-sm font-bold lg:-mt-14">{name}</h1>
            <h1 className="italic text-sm md:text-lg">{nickname}</h1>
          </div>
        </div>
        <div>
